...
makePaperPlot2 <- function(model_arg, mcmcs, thiswidth, thisBarWidth, thisdf) {
    ...
    p3 <- ggplot(thisdf, aes(mcmc, Efficiency, colour=mcmc, fill=mcmc), xlab='') + geom_boxplot(width=0.8, colour='black') + ggtitle('Boxplot') + ylab('Sampling efficiency (ESPS)') + theme(legend.position='none', axis.title.x=element_blank(), axis.text.x=element_blank(), axis.ticks.x = element_blank())
